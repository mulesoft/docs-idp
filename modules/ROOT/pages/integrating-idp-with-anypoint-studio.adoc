...
Execute a published document action and retrieve the results from Anypoint Studio using the IDP Runtime Service API connector. This connector generates automatically when you publish a document action to Anypoint Exchange.

Each document action version generates a different connector. For example, when you publish a document action for the first time, it generates a connector in Exchange that calls version `1.0.0` of this document action. Then, if you modify the document action and republish it as version `1.1.0`, it generates a different connector that you must download and install in Studio to call this new document action version. Consider this behavior when building your integrations. 

The IDP REST connector has the following operations: 
...
* *host*: the Anypoint endpoint URL. 
+
For example: `https://idp-rt.{region}.anypoint.mulesoft.com`.
* *port*: the HTTP port to use for the connection.
+
For example: `443`.
* *basePath*: the base path of the URL to call, without the protocol. 
* *protocol*: the protocol to use for the call, in upper case. 
+
For example: `HTTPS`. 
* *Response timeout*: the response timeout for this operation. 
* *clientId*: Client ID of the connected app to use to call IDP. 
* *clientSecret*: Client Secret of the connected app to use to call IDP. 
* *accessTokenUrl*: the URL to obtain the access token for your connected app. 
+
...
The response from this operation contains the execution ID that you must use to retrieve the results of the execution. 
...
Following is a sample response from this operation, shortened for better readability: 
