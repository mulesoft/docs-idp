...
You can execute your published document actions and query the results by using the Send Document and Retrieve Results action steps from MuleSoft RPA. Because RPA calls the IDP API in the background, you don't need to configure a connected app and call the API to execute document actions. However, you still need to publish your document actions to make them available to RPA. 
