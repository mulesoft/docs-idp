...
IDP provides preconfigured Invoice and Purchase Order templates, which you can select when creating custom document actions. Each template provides a set of expected fields to extract from the document and generate a structured response. When creating a new document action, choose the type that better aligns with the document to process. For example, the Invoice type can process invoices and similar documents such as receipts. If none of these types align with your documents, use the Generic type to fully customize the fields to extract by asking questions in natural language. 
