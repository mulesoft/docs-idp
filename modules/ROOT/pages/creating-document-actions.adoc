...
By default, IDP uses its natural language processing model (IDP NLP) to extract data based on the configured prompts. When you create a document action, you can specify in each prompt the model that analyzes the document and extracts the data.

=== Before You Begin Using Einstein

Ensure you completed these configuration steps:

. https://help.salesforce.com/s/articleView?id=platform.external_services_setup_mulesoft_connect_to_anypoint_exchange.htm&type=5[Connect Salesforce to MuleSoft Anypoint Exchange from Setup]
...
. <<upload-files, Create a new document action>> or edit an existing one. 
. <<add-prompts, Add a new prompt to the document action and run a document analysis>>.
. Find the results in the *Outputs* panel. 
...
For examples, see xref:example-einstein-prompts.adoc[].
