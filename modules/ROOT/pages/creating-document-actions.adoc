...
To analyze a generic document and fully customize the output fields and tables instead of using pre-built schemas, see xref:analyzing-documents-with-einstein.adoc[].
...
. <<configure-schema>> 
+
Unless you select the Generic document action, which does not use a schema.
. <<add-prompts>>
