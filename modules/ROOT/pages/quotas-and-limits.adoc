...
* 50 pages per file
* 8 MB per file
** 3 MB per file when using image recognition.
+
Uploading image files larger than 3 MB might cause errors.  
! 
...
* 512,000 characters per request for Einstein prompts
+
--
** 120,000 characters per request for Einstein prompts when PII masking is enabled
--
+
The total number of characters in a request includes: 
+
...
** Approximately 4,000 characters that IDP uses to process the request
...
* Approximately 6,000 output words for Einstein prompts using `Einstein GEMINI-20 Flash 001`
| Request Limits
a| 
...
| Document Languages 
a| 
Full English, Spanish, German, French, Italian, and Portuguese document support +
...
a| * Checkboxes
+
Reading checkboxes in documents is not supported. IDP doesn't guarantee the accuracy of any results related to interpreting checkbox values.
|===
