...
[IMPORTANT]
====
Resubmitting a document after a `FAILED` or `PARTIAL_SUCCESS` execution creates a new execution and consumes Automation Credits in full, in addition to the credits already consumed by the original execution. See xref:quotas-and-limits.adoc#automation-credits[Automation Credit Consumption and Estimation].
====
